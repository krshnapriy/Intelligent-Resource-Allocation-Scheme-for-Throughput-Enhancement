...
def trans_rate(user_loc, edge_loc):
    B = 2e6                                                              #Bandwidth of the channel
    P = 0.25                                                             #Signal power
    d = np.sqrt(np.sum(np.square(user_loc[0] - edge_loc))) + 0.01        #Euclidean distance between the user and the edge server
    h = 4.11 * math.pow(3e8 / (4 * math.pi * 915e6 * d), 2)              #channel gain
    N = 1e-10                                                            #noise power in the communication channel
    return B * math.log2(1 + P * h / N)     #represents the capacity of a communication channel in a wireless communication system


def BandwidthTable(edge_num):
    BandwidthTable = np.zeros((edge_num, edge_num))           # Represent the bandwidth table between edge servers.
    for i in range(0, edge_num):
        for j in range(i+1, edge_num):
                BandwidthTable[i][j] = 1e9
    return BandwidthTable

def two_to_one(two_table):                         #Flattens a 2D array (two_table) into a 1D array (one_table).
    one_table = two_table.flatten()
    return one_table

def generate_state(two_table, U, E, x_min, y_min):      #Generates the state representation for the reinforcement learning environment.
    one_table = two_to_one(two_table)
    S = np.zeros(len(E) + one_table.size + len(U) + len(U)*2 + len(U) +  1 + len(E))
    ...
    #data transmitted
    for user in U:
        S[count] = user.req.e2u_size
        count += 1   

    # Required data rate of each user/service
    #for user in U:
    #    S[count] = user.req.tasktype.req_u2e_size       
    #    count += 1

    # current load of the network - number of connected users
    network_load = len([user for user in U if user.req.state != 5]) / len(U)
    S[count] = network_load
    count += 1
  
    ##CPU utilization of each edge server 
    for edge in E: 
        S[count] = (edge.capability - edge.resource) / edge.capability 
        count += 1
    
    #### Load utilization of each edge server (for migration tasks)
    #for edge in E:
    #    total_capability = edge.capability
    #    current_utilization = total_capability - sum([U[user_id].req.resource for user_id in edge.user_group])
    #    S[count] = current_utilization/total_capability
    #    count += 1

    return S

def generate_action(R, B, O, CPU): ### CPU parameter added extra
    # resource
    ...
    ## CPU utilization ####additional
    a[USER_NUM + USER_NUM + EDGE_NUM * USER_NUM:] = CPU 
    #############
...
class UE():
    ...
    def request_update(self):
        ...
        if self.req.state == 5:             #self.req.state -> current state of the request
            self.req.timer += 1
        else:
            self.req.timer = 0
            if self.req.state == 0:
                ...
                if self.req.process_size > 0:
                    ...
                    self.req.e2u_size -= np.random.randint(1,10000) # value is small,so simplify
                    
            else:
                if self.req.e2u_size > 0:
                    self.req.e2u_size -= np.random.randint(1,10000)  # B*math.log(1+SINR(self.user.loc, self.offloading_serv.loc), 2)/(8*time_scale)
...
class EdgeServer():
    def __init__(self, edge_id, loc):
        ...
        self.resource = 0 ## Additional attribute for CPU utilization
    ...
    def maintain_request(self, R, U):
        ...
        ## Update CPU utilization #############additional
        self.resource = sum(R[user_id] for user_id in self.user_group)
        ###########
...
class Env():
    ...
    def calculate_throughput(self):
        total_throughput = 0
        for user in self.U:
            if user.req.state == 3:  # Completed request
                # Calculate the data transmitted in this time step
                data_transmitted = user.req.tasktype.req_e2u_size - user.req.e2u_size

                #data_transmitted += user.req.mig_size   #to add transmission involved during migration
                
                # Add the data transmitted to the total throughput
    ...
    def ddpg_step_forward(self, a, r_dim, b_dim):
        ...
        for user_id in range(self.user_num):
            ...
            prob_weights_new = np.nan_to_num(prob_weights, copy=True)
            prob_weights_sum = np.sum(prob_weights_new)
            if prob_weights_sum == 0:
                # If all probabilities are NaN, assign equal probabilities
                prob_weights_new = np.ones_like(prob_weights_new) / len(prob_weights_new)
            else:
                # Normalize probabilities to sum up to 1
                prob_weights_new /= prob_weights_sum

            # Select offloading action w.r.t the actions prob
            action = np.random.choice(range(self.edge_num), p=prob_weights_new.ravel())  
            base += self.edge_num
        ...
        for user in self.U:
            ...
            if user.req.state == 4:
                ...
                user.req.state = 5  # Request turned to "disconnect"
        ...
        # Collect edge capabilities for this episode
        edge_capability_sum = 0
       
        offload = 0
        migration_size = 0

        for edge in self.E: 
            edge_capability_sum += edge.capability

        for user in self.U: #number of edges performing offloading (migration) - edge to edge
              if user.req.state == 6:
                offload += 1

        for user in self.U: #migration size
            migration_size += user.req.mig_size

        total_users_served= self.fin_req_count - self.prev_count
        print(total_users_served)
        # Calculate throughput for this time step
        throughput = self.calculate_throughput()
        # Set rewards as the throughput (maximizing throughput)
      
        self.rewards = throughput * 0.7 + edge.capability * 0.3
        self.prev_count = self.fin_req_count    

        ####################################################################
        
        # Every user starts to move
        ...
        # Return state (s_), reward (self.rewards)
        return generate_state(self.table, self.U, self.E, self.x_min, self.y_min), self.rewards, edge_capability_sum, throughput,offload, migration_size

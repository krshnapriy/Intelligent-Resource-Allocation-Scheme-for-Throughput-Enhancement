...
def generate_state(two_table, U, E, x_min, y_min):      #Generates the state representation for the reinforcement learning environment.
    ...
    S = np.zeros((len(E) + one_table.size + len(U) + len(U)*2) + len(U))
    # transform
    count = 0
...
class Env():
    ...
    def ddpg_step_forward(self, a, r_dim, b_dim):
        ...
        for user_id in range(self.user_num):
            ...
            action = np.random.choice(range(len(prob_weights)), p=prob_weights.ravel())  # Select action based on probabilities
            base += self.edge_num
        ...
        return generate_state(self.table, self.U, self.E, self.x_min, self.y_min), self.rewards, edge_capability_sum, throughput

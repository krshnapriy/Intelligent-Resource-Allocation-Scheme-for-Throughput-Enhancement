...
if __name__ == "__main__":
    ...
    ep_throughput = []
    r_v, b_v = [], []
    episode_edge_capabilities = []  # Store edge capabilities for each episode
    var_reward = []
    max_rewards = 0
    episode = 0
    var_counter = 0
    ...
    while var_counter < LEARNING_MAX_EPISODE:
        ...
        episode_edge_capabilities.append(0)
        if SCREEN_RENDER:
        ...
        for j in range(MAX_EP_STEPS):
            ...
            s_, r, edge_capability_sum, throughput = env.ddpg_step_forward(a, r_dim, b_dim)  

            print("EPISODE:", episode)
            print("EDGE CAPABLITY SUM:", edge_capability_sum)
            ...
            if j == MAX_EP_STEPS - 1:
                ...
                f1.write("edge capability :"+str(episode_edge_capabilities[episode]))
                f1.close()
                ##################
                ...
                print('Episode:%3d' % episode, ' Reward: %5d' % ep_reward[episode], '###  r_var: %.2f ' % r_var,
                      'b_var: %.2f ' % b_var, )
                print('Edge Capabilities:', episode_edge_capabilities)
    ...
    plt.plot([i + 1 for i in range(episode)], episode_edge_capabilities)
    plt.xlabel("episode")

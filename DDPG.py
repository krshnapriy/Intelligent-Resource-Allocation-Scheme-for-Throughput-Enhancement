...
class DDPG(object):
    def __init__(self, s_dim, r_dim, b_dim, o_dim, r_bound, b_bound):
        ...
        r_dim: dimension of the resource allocation vector.
        b_dim: dimension of the bandwidth allocation vector.
        o_dim: dimension of the offload decision vector.
        r_bound: upper bound on the resource allocation values.
        b_bound: upper bound on the bandwidth allocation values.
        """

        self.memory_capacity = 10000        # sets the size of the experience replay memory to 10,000 transitions.

        # store the dimensions of the s, a, r, b, o vectors.
        self.s_dim = s_dim                        
        ...
        # store the upper bounds on the resource allocation and bandwidth allocation values.
        self.r_bound = r_bound
        self.b_bound = b_bound

        # S, S_, R -> Define three placeholders for the state, next state, and reward. 
        # These placeholders will be used to feed data to the actor and critic networks during training.
        self.S = tf.placeholder(tf.float32, [None, s_dim], 's')
        self.S_ = tf.placeholder(tf.float32, [None, s_dim], 's_')
        self.R = tf.placeholder(tf.float32, [None, 1], 'r')

        # creates an empty experience replay memory with a capacity of 10,000 transitions.
        self.memory = np.zeros((self.memory_capacity, s_dim * 2 + self.a_dim + 1), dtype=np.float32)                        # s_dim + a_dim + r + s_dim
        self.pointer = 0            # keep track of the next position in the experience replay memory.
        
        # creates a TensorFlow session - responsible for executing the operations defined in the TensorFlow graph. 
        self.sess = tf.Session()

        # define the input and output
        # builds and defines the actor network, which is responsible for selecting actions.
        self.a = self._build_a(self.S,)     
        # builds and defines the critic network, which is responsible for evaluating the value of actions.

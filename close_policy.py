...
def generate_state( R, O, two_table, U):
    ...
    count = 0
    # R
    for user_id in range(len(R)):
        S[count] = R[user_id]
        count += 1
    # O
    for user_id in range(len(O)):
        S[count] = O[user_id]
        count += 1
    # bandwidth
    ...
    # user's location
    for user in U:
        S[count] = user.loc[0][0]
        count += 1
        S[count] = user.loc[0][1]
        count += 1
    return S
...
class close_policy():
    def offloading_update(self, O, U, E):
        for user in U:
...
class Env():
    ...
    def reset(self):
        ...
        """
        # model
        self.model = close_policy()

        # initialize the request
        self.O = self.model.offloading_update(self.O, self.U, self.E)
        for user in self.U:
            user.generate_request(self.O[user.user_id])

    def close_step_forward(self):
...
if __name__ == "__main__":
    ...
    # make directory
    dir_name = "close_" + str(USER_NUM) + 'u' + str(EDGE_NUM) + 'e' + str(LIMIT) + 'l' + LOCATION
